...
try {
  ...
  (function UseLetOrConst() {
    ...
    const x = 1;
    if (true) {
      const x = 2; // missing Const as constant value otherwise a error of undefined
    }
  ...
  (function UseTemplateStrings1() {
    ...
    // Remember Backthick ` is not same as single quote '
    // Rewrite the line below to use template literals.
    const greeting = `Hello ${who}! ${addOne(2)} times.`; // sense it says 3 times, will add 2.
  ...
  /// Rest parameter /// (remember it is a rest and not a spread.)
  ...
  (function UseRestParameter() {
    ...
    function foo(a, b, ...c) {
      // sense modern JS can know which index its is, we just put one more value.
      return a + b + c.length; // length of array.
    }
  ...
  (function UseSpreadOperator1() {
    function add(a, b, c) {
      // they are alredy written in correct
      return a + b + c;
    }

    let values = [1, 3, 6]; // also given value

    // Use spread operator to let the test pass.
    let result = add(...values); // only need for spread , oppsite of rest
  ...
  (function UseSpreadOperator2() {
    const arr1 = [1, 2, 3];
    const value = 4; // ikke liste then no need of spread .
    const arr2 = [5, 6];
  ...
  (function UseArrow1() {
    ...
    expect(double(3)).toBe(6); // expected to be 6 , 2 * 3
  ...
  (function UseArrayDestructuring1() {
    ...
    const [a, , b, ...c] = arr; // remember to put ,, otherwise it wil count as recived 2 and not 3.
    //console.log(arr);
  ...
  (function UseArrayDestructuring2() {
    ...
    // can be written like the swapping and more cleaner:
  ...
  (function UseObjectDestructuring1() {
    let obj = {
      name: "Oslo",
      age: 985,
      add: (x, y) => x + y // expected value 1 + 2 = 3,
      // consol.log (name,age, add);
    };
    ...
    let { name, age, add } = obj; // important with order compared to above
  ...
  (function UseParameterDestructuring1() {
    ...
    const a = ["Oslo", 985]; // Two things one is [] and ,
  ...
  (function UseParameterDestructuring2() {
    ...
    const a = { name: "Oslo", age: 985 }; // {}, her have to spesify the parametes
  ...
  // Kommentar; Har prøvd en del å prøve å løse den, jeg vet i hverfall hvordan man løse med assign biten.
